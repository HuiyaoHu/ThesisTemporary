...
const obsLight = new THREE.MeshStandardMaterial({color: 'grey'});
...
const clt = new THREE.MeshStandardMaterial({color: 'gainsboro', side: THREE.DoubleSide}); //door
...
const geomVolumeBase = new THREE.PlaneGeometry( 24 * volume_width, 4*volume_width); // width, height
...
const num_cells = 4; 
const ground_width = num_cells * volume_width;
const ground_length = 24 * volume_width;

const ground_geo = new THREE.PlaneBufferGeometry( ground_length, ground_width); // width, height
const ground = new THREE.Mesh( ground_geo, matBdyWallDel );
...
// GrpMeshNeighbours.visible = true;
...
var dictOpeningWall = {};
const tol_betweenOpeningtoBdyMesh = 0.15 // tolerance
...
loader.load( // Load a glTF resource
    ...
    function ( gltf ) { // called when the resource is loaded
        ...
        grpWindow02.getObjectByName("WindowFrame").material = brass;
...
// DIMENSIONS
const Window03_width = PartWall_width*3; const Window03_width_half = Window03_width / 2;
const Window03_height = 3.5; const Window03_height_half = Window03_height / 2;
const Window03_thickness = 0.5;

const geomWindow03Hover = new THREE.BoxBufferGeometry( Window03_width * 1, BdyWall_thickness* 2, BdyWall_height * 1 );
const geomWindow03Del = new THREE.BoxBufferGeometry( Window03_width * 1.1, BdyWall_thickness* 3, BdyWall_height * 1.1 );

const meshWindow03Trans = new THREE.Mesh( geomWindow03Hover, matHybridTrans );
meshWindow03Trans.visible = false;
const meshWindow03Del = new THREE.Mesh( geomWindow03Del, matBdyWallDel );
meshWindow03Del.visible = false;

scene.add(meshWindow03Trans);
scene.add(meshWindow03Del);

// GEOMETRIES
var  meshWindow03 = null
loader.load( // Load a glTF resource
    'models/Window03.gltf', // resource URL
    function ( gltf ) { // called when the resource is loaded
        meshWindow03 = gltf.scene;
        meshWindow03.getObjectByName("WindowSeal").material = rubber;
        meshWindow03.getObjectByName("SIP").material = obs;
        meshWindow03.getObjectByName("Glass").material = glass;
        meshWindow03.getObjectByName("WindowFrame").material = aluminium;
    },
    function ( xhr ) { // called while loading is progressing
		console.log( ( xhr.loaded / xhr.total * 100 ) + '% loaded' );
	},
...
// INITIALISATION
var dictWindow03 = {};
var pos_grpWindow03 = null;
var bool_delWindow03 = false;
var cnt_grpWindow03 = 0;
var angle_grpWindow03 = 0;
...
// DIMENSIONS
const Window04_width = PartWall_width*2; const Window04_width_half = Window04_width / 2;
const Window04_height = 3.5; const Window04_height_half = Window04_height / 2;
const Window04_thickness = 0.5;

const pt_axis = dispDotsfromCoords(matAttrDot, [ new THREE.Vector3(0,0,0)] )

const geomWindow04Hover = new THREE.BoxBufferGeometry( Window04_width * 1, BdyWall_thickness* 2, BdyWall_height * 1 );
const meshWindow04Trans = new THREE.Mesh( geomWindow04Hover, matHybridTrans );
meshWindow04Trans.position.set( -PartWall_width_half, 0, 0 );
const GrpWindow04Trans = new THREE.Group();
GrpWindow04Trans.add( meshWindow04Trans, pt_axis );
scene.add(GrpWindow04Trans);
GrpWindow04Trans.visible = false;

const geomWindow04Del = new THREE.BoxBufferGeometry( Window04_width * 1.1, BdyWall_thickness* 3, BdyWall_height * 1.1 );
const meshWindow04Del = new THREE.Mesh( geomWindow04Del, matBdyWallDel );
meshWindow04Del.position.set( -PartWall_width_half, 0, 0 );
const GrpWindow04Del = new THREE.Group();
GrpWindow04Del.add( meshWindow04Del, pt_axis );
scene.add(GrpWindow04Del);
GrpWindow04Del.visible = false;



// GEOMETRIES
var  meshWindow04 = null
loader.load( // Load a glTF resource
    'models/Window04.gltf', // resource URL
    function ( gltf ) { // called when the resource is loaded
        meshWindow04 = gltf.scene;
        meshWindow04.getObjectByName("SIP").material = obs;
        meshWindow04.getObjectByName("Glass").material = glass;
        meshWindow04.getObjectByName("WindowFrame").material = brass;
        meshWindow04.getObjectByName("WindowCasement").material = alloy;
        meshWindow04.getObjectByName("SlidingWindowHandle").material = brass;
    },
    function ( xhr ) { // called while loading is progressing
		console.log( ( xhr.loaded / xhr.total * 100 ) + '% loaded' );
	},
...
// INITIALISATION
var dictWindow04 = {};
var pos_grpWindow04 = null;
var bool_delWindow04 = false;
...
loader.load( // Load a glTF resource
    ...
    function ( gltf ) { // called when the resource is loaded
        ...
        meshStairs01.getObjectByName("Tread").material = alloy;
        meshStairs01.getObjectByName("Baluster").material = alloy;
        // meshStairs01.getObjectByName("Rail").material = alloy;
    },
    function ( xhr ) { // called while loading is progressing
		console.log( ( xhr.loaded / xhr.total * 100 ) + '% loaded' );
	},
...
function render() {
    // const testmesh = test.clone();
    // scene.add(test)
    // const test_key = 'test'
    // dictTest[test_key] = test;
    // test.storey=1    
   ...
   // __________________________
    ...
    if ( id_buttonPressed == 'buttonStairs01' ) {
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Stairs01') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delStairs01) { // if shift button is not pressed, update trans pos and show geom_trans 
                    // orientVertMeshToFaceOfIntMesh( meshInt0, pos_meshPartWall, meshPartWallTrans, meshPartWallDel );
                }
                else { // if shift button is pressed, show geom_trans_del
                    const grpInt = meshInt0.object.parent.parent; // centre of the first mesh that the cursor intersects, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
                ...
                if (!bool_delStairs01) { // if shift button is not pressed, update pos and show geom_trans
    ...
    //    	　_ * WINDOW03 * ★
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
    if ( id_buttonPressed == 'buttonWindow03' ) {
        const list_meshInt = getListofMeshInt();
        const meshInt0 = list_meshInt[ 0 ]; // get the first mesh that the cursor intersects e.g. {distance: 29.318, point: Vector3, object: Mesh, face: Face3, faceIndex: 5}
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            if (meshInt0.object.parent.name == 'Window03') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delWindow03) { // if shift button is not pressed, update global variable of geom & geom_trans
                    const meshInt = meshInt0.object
                    dispHelperAndReplaceHover_wallModRule (replaceHoverDisp_Window03, meshInt);
                }
            }

            else if (meshInt0.object.name == 'OpeningWall' ) { // if the first mesh that the cursor intersects has the name ' '
                if (!bool_delWindow03) { // if shift button is not pressed, update global variable of geom & geom_trans
                    replaceHoverDisp_Window03( meshInt0.object );
                }
            }
    ...
    //    	　_ * WINDOW04 * ★
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
    if ( id_buttonPressed == 'buttonWindow04' ) {
        const list_meshInt = getListofMeshInt();
        const meshInt0 = list_meshInt[ 0 ]; // get the first mesh that the cursor intersects e.g. {distance: 29.318, point: Vector3, object: Mesh, face: Face3, faceIndex: 5}
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Window04') { // if the first mesh that the cursor intersects has the name ' '
                const grpInt = meshInt0.object.parent.parent; // centre of the first mesh that the cursor intersects, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
                if (!bool_delWindow04) {
                    // if shift button is not pressed, do nothing
                } 
                ...
                if (!bool_delWindow04) { // if shift button is not pressed, update global variable of geom & geom_trans
                    const meshInt = meshInt0.object
                    dispHelperAndReplaceHover_wallModRule (replaceHoverDisp_Window04, meshInt);
                }
            }

            else if (meshInt0.object.name == 'OpeningWall' ) { // if the first mesh that the cursor intersects has the name ' '
                if (!bool_delWindow04) { // if shift button is not pressed, update global variable of geom & geom_trans
                    replaceHoverDisp_Window04( meshInt0.object );
                }
            }
...
//    	　_ * WINDOW03 * 
// ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
function onClickbuttonWindow03() {
    document.getElementById(id_buttonPressed).classList.remove("pressed");
    id_buttonPressed = 'buttonWindow03';
    document.getElementById(id_buttonPressed).classList.add("pressed");
};

//    	　_ * WINDOW04 * 
// ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
function onClickbuttonWindow04() {
    document.getElementById(id_buttonPressed).classList.remove("pressed");
...
function onMouseMove( event ) {
    ...
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
    if (event.shiftKey) {
        bool_delWindow03 = true;
    } else {
        bool_delWindow03 = false;
    }
...
function onMouseUp(event) { // Mouse up: do nothing, create mesh or delete mesh
    ...
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
        ...
        if (bool_delStairs01 &&key in dictStairs01) { // if shift is pressed and existing key is True, delete
            addFloorFrac_deleteStairs(dictStairs01[key]);
            deleteStairs01(dictStairs01[key]); 


        } else if (!bool_delStairs01 && dictStairs01[key]==undefined) { //if shift is not pressed and there is no exisitng key, add mesh and add its key to dict {}
            addStairs01(key); 
            deleteFloorFrac_addStairs(key)
            // createStairsAccess(key);
            // dispDotsfromCoords(matAttrDot_Large,[pos_grpStairs01])
        }
    }
    // __________________________
    //    	　_ * FLOOR FRACTION *
...
*/
    ...
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
        ...
        MouseUpDisplay_onHybridMod_1unit('Window01', pos_grpWindow01, angle_grpWindow01, bool_delWindow01, dictWindow01, deleteWindow01, addWindow01);
    }
    else if (pos_grpWindow02 != null) {
        MouseUpDisplay_onHybridMod_1unit('Window02', pos_grpWindow02, angle_grpWindow02, bool_delWindow02, dictWindow02, deleteWindow02, addWindow02);
    }
    else if (pos_grpWindow03 != null) {
        MouseUpDisplay_onHybridMod_3unit('Window03', pos_grpWindow03, angle_grpWindow03, bool_delWindow03, dictWindow03, deleteWindow03, addWindow03);
    }
        ...
        MouseUpDisplay_onHybridMod_1unit('Door02', pos_grpDoor02, angle_grpDoor02, bool_delDoor02, dictDoor02, deleteDoor02, addDoor02);
    }
    else if (pos_grpDoor03 != null) {
        MouseUpDisplay_onHybridMod_3unit('Door03', pos_grpDoor03, angle_grpDoor03, bool_delDoor03, dictDoor03, deleteDoor03, addDoor03);
    }
    else if (pos_grpRailing01 != null) {
        MouseUpDisplay_onHybridMod_1unit('Railing01', pos_grpRailing01, angle_grpRailing01, bool_delRailing01, dictRailing01, deleteRailing01, addRailing01);
    }
    
}

/*
function createStairsAccess(key){
    pos_grpStairs01
    const pos_LowerOpening = pos_grpStairs01.clone().add(new THREE.Vector3(FloorFrac_width_half,0,PartWall_height_half));
    const pos_UpperOpening = pos_grpStairs01.clone().add(new THREE.Vector3(FloorFrac_width_half,0,volume_height+PartWall_height_half));
    console.log(pos_grpStairs01, pos_LowerOpening,pos_UpperOpening,dictOpeningWall)
...
function addOpeningWall(key, pos, angle, DefaultAttr = true) {
    ...
    */
    ...
    cnt_meshPartWall += 1; 
    document.getElementById('buttonPartitionWall').innerHTML = "Interior Wall: " + cnt_meshPartWall;
...
function addBdyWall(key, pos, angle, DefaultAttr = true) {
    ...
    */
    ...
    cnt_meshBdyWall += 1; 
    document.getElementById('buttonBoundaryWall').innerHTML = "Exterior Wall: " + cnt_meshBdyWall;
...
function addWindow03(key) {

    // ADD MESH
    const mesh = meshWindow03.clone();
    mesh.position.set(pos_grpWindow03.x, pos_grpWindow03.y, pos_grpWindow03.z);
    scene.add( mesh );
    ...
    // UPDATE GLOBAL VARIABLES, HTML
    dictWindow03[key] = mesh;
    const list_2AdjKeys = get2AdjKeys(angle_grpWindow03, pos_grpWindow03)
    list_2AdjKeys.forEach(function(adj_key){ 
        dictWindow03[adj_key] = mesh;
    })
    mesh.rotation.z = angle_grpWindow03;
    cnt_grpWindow03 += 1; 
    document.getElementById('buttonWindow03').innerHTML = "Large Fixed Window: " + cnt_grpWindow03;
};

// Delete a Window03
function deleteWindow03(mesh) {
    scene.remove( mesh );
    delete dictWindow03[ mesh.key ];
    const list_2AdjKeys = get2AdjKeys(mesh.rotation.z, mesh.position)
    list_2AdjKeys.forEach(function(adj_key){ 
        delete dictWindow03[adj_key];
    })
    cnt_grpWindow03 -= 1; 
    document.getElementById('buttonWindow03').innerHTML = "Large Fixed Window: " + cnt_grpWindow03;
};
...
function addWindow04(key) {

    // ADD MESH
    const mesh = meshWindow04.clone();
    mesh.position.set(pos_grpWindow04.x, pos_grpWindow04.y, pos_grpWindow04.z);
    scene.add( mesh );
    ...
    // UPDATE GLOBAL VARIABLES, HTML
    dictWindow04[key] = mesh;
    const adj_key = keyGen( checkPresenceAndPosOf1AdjacentMeshes(pos_grpWindow04, angle_grpWindow04) );
        dictWindow04[adj_key] = mesh;
    mesh.rotation.z = angle_grpWindow04;
    cnt_grpWindow04 += 1; 
    document.getElementById('buttonWindow04').innerHTML = "Sliding Window: " + cnt_grpWindow04;
};

// Delete a Window04
function deleteWindow04(mesh) {
    scene.remove( mesh );
    delete dictWindow04[ mesh.key ];
    const adj_key = keyGen(  checkPresenceAndPosOf1AdjacentMeshes(mesh.position, mesh.rotation.z) );
...
function addDoor03(key) {
    ...
    // UPDATE GLOBAL VARIABLES, HTML
    dictDoor03[key] = mesh;
    const list_2AdjKeys = get2AdjKeys(angle_grpDoor03, pos_grpDoor03)
    list_2AdjKeys.forEach(function(adj_key){ 
        dictDoor03[adj_key] = mesh;
    })
...
function changeGroundHeight() { // update storeyNum, groundHeight and grid display
    ...
    Grid01.position.z = groundHeight;  
...
// --------------------------------
//    Add FloorOpening Hover/Del Display  
// --------------------------------
...
function addHoverDisp_PartWall_toIntFloor(meshInt0) { // check distance from boundary and presense of boundary mesh
    ...
    const pos_AttrLine_extrap = new THREE.Vector3( pos_cursor.x, pos_cursor.y, zpos_AttrLine )
    const list_posAttrLine = getPos_allAttrLine ();
    const pos_closestAttrLine = getClosestPos (pos_AttrLine_extrap, list_posAttrLine); // CAN BE OPTIMISED
...
/*
// replaceHoverDisp_1Unit( meshInt0.object, pos_grpWindow01, angle_grpWindow01, meshWindow01Trans, meshWindow01Del);

function replaceHoverDisp_1Unit( mesh, pos, angle, meshTrans, meshDel){
    pos = new THREE.Vector3(mesh.position.x, mesh.position.y, mesh.position.z);
    angle = mesh.rotation.z;
    
    meshTrans.position.copy(pos);
    meshTrans.rotation.z = angle;
    meshTrans.visible = true;
    meshDel.visible = false;
}
*/
...
function replaceHoverDisp_Window03( mesh ) { // copy pos, angle, show meshTrans
    pos_grpWindow03 = new THREE.Vector3(mesh.position.x, mesh.position.y, mesh.position.z);
    angle_grpWindow03 = mesh.rotation.z;

    const bool_PresenceOf2AdjacentMeshes = checkPresenceOf2AdjacentMeshes(pos_grpWindow03, angle_grpWindow03)
    if (bool_PresenceOf2AdjacentMeshes) {
        meshWindow03Trans.position.copy(pos_grpWindow03);
        meshWindow03Trans.rotation.z = angle_grpWindow03
        meshWindow03Trans.visible = true;
        meshWindow03Del.visible = false;
    }

}

function replaceHoverDisp_Window04( mesh ) { // copy pos, angle, show meshTrans
    angle_grpWindow04 = mesh.rotation.z;
    const angle = angle_grpWindow04
    // console.log(angle)
    if (angle == 0 ||  angle == -Math.PI){ // if mesh is horizontal
        pos_grpWindow04 = new THREE.Vector3(mesh.position.x, mesh.position.y, mesh.position.z);
    ...
    const bool_PresenceOf1AdjacentMeshes = checkPresenceAndPosOf1AdjacentMeshes(pos_grpWindow04, angle_grpWindow04) !==undefined
    if (bool_PresenceOf1AdjacentMeshes) {
        const pos_adj = checkPresenceAndPosOf1AdjacentMeshes(pos_grpWindow04, angle_grpWindow04);
        const key_adj = keyGen( pos_adj );
        if (dictWindow04[key_adj] ==undefined){
            GrpWindow04Trans.position.copy(pos_grpWindow04);
            GrpWindow04Trans.rotation.z = angle_grpWindow04
            GrpWindow04Trans.visible = true;
            GrpWindow04Del.visible = false;
        }
    }
...
function delHoverDisp_Window03( mesh ) { // copy pos, angle, show meshTrans
    pos_grpWindow03 = new THREE.Vector3(mesh.position.x, mesh.position.y, mesh.position.z); // centre of the first mesh that the cursor intersects, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
    angle_grpWindow03 = mesh.rotation.z;

    meshWindow03Del.position.copy(pos_grpWindow03);
    meshWindow03Del.rotation.z = angle_grpWindow03;
    meshWindow03Del.visible = true;
    meshWindow03Trans.visible = false;
}

function delHoverDisp_Window04( mesh ) { // copy pos, angle, show meshTrans
    pos_grpWindow04 = new THREE.Vector3(mesh.position.x, mesh.position.y, mesh.position.z); // centre of the first mesh that the cursor intersects, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
    angle_grpWindow04 = mesh.rotation.z;

    GrpWindow04Del.position.copy(pos_grpWindow04);
    GrpWindow04Del.rotation.z = angle_grpWindow04;
    GrpWindow04Del.visible = true;
    GrpWindow04Trans.visible = false;
}
...
function reinstate_mods() { //  no display of trans or del mesh, clear position
    ...
    if (args.includes('BoundaryWall')) { reinstate_BdyWall() };
...
function reinstate_Window03() {
    meshWindow03Trans.visible = false;
    meshWindow03Del.visible = false;
    pos_grpWindow03 = null;
}
function reinstate_Window04() {
    GrpWindow04Trans.visible = false;
    GrpWindow04Del.visible = false;
...
function MouseUpDisplay_onHybridMod_1unit(name_HybridMesh, pos_HybridMesh, angle_HybridMesh, bool_delHybridMesh, dictHybridMesh, deleteHybridMesh, addHybridMesh) { 
    ...
    if (bool_delHybridMesh && key in dictHybridMesh) { // if shift is pressed and existing key is True, delete mesh and its key
        const adjacency = getAttrLineAdjacencyAtPos(pos_HybridMesh);
        if (adjacency == 'interior') { // if pos is for partition wall
...
function MouseUpDisplay_onHybridMod_3unit(name_HybridMesh, pos_HybridMesh, angle_HybridMesh, bool_delHybridMesh, dictHybridMesh, deleteHybridMesh, addHybridMesh) { 
    const list_keys = get2AdjKeys(angle_HybridMesh, pos_HybridMesh);
    const mesh_key = keyGen(pos_HybridMesh);
    list_keys.push(mesh_key);

    if (bool_delHybridMesh && mesh_key in dictHybridMesh) { // if shift is pressed and existing key is True, delete mesh and its key
        const adjacency = getAttrLineAdjacencyAtPos(pos_HybridMesh);
        if (adjacency == 'interior') { // if pos is for partition wall
            list_keys.forEach(function(key){
                addPartWall(key, posfromKey(key), angle_HybridMesh);
            })
        } else { // if pos is bdy wall
            list_keys.forEach(function(key){
                addBdyWall(key, posfromKey(key), angle_HybridMesh); 
            })
        }
...
function MouseUpDisplay_onHybridMod_2unit(name_HybridMesh, pos_HybridMesh, angle_HybridMesh, bool_delHybridMesh, dictHybridMesh, deleteHybridMesh, addHybridMesh) { 

    const list_keys = [keyGen(checkPresenceAndPosOf1AdjacentMeshes(pos_HybridMesh, angle_HybridMesh))]; //only 1 returned
    const mesh_key = keyGen(pos_HybridMesh);
    list_keys.push(mesh_key);
...
function addWalls_replacingMultipleUnitHybridMod_With1Unit(key, list_dict, pos_HybridMesh, angle_HybridMesh){
    var list_keys = [];//to be overwritten
    list_dict.forEach(function(dict){
        if (key in dict) {
            const value = dict[key];
            list_keys = getListofKeysWithValue( value, dict );
        }
    })
    const adjacency = getAttrLineAdjacencyAtPos(pos_HybridMesh);
    if (adjacency == 'interior') { // if pos is for partition wall
...
//    Get Key of Value
// --------------------------------
function getListofKeysWithValue( value, dict ){
    const list_keys =[]
...
// --------------------------------
//    Check Presence of 1 Adjacent Meshes
// --------------------------------

function checkPresenceAndPosOf1AdjacentMeshes( pos, angle ) {
    // dispDotsfromCoords(matAttrDot_Large, [pos])
    var bool_PresenceOf1AdjacentMeshes = false;
    var pos_adjMesh = undefined;
    if (angle == 0 || angle == -Math.PI ){ // if mesh is horizontal
        const pos_lefttMesh = pos.clone().add(new THREE.Vector3(-PartWall_width, 0 ,0) );
        pos_adjMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_lefttMesh, tol_01);
    } else if (angle == Math.PI/2) { // if mesh is vertical
        const pos_topMesh = pos.clone().add(new THREE.Vector3(0, -PartWall_width,0) );
        pos_adjMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_topMesh, tol_01);
    } else if (angle== Math.PI) { 
        const pos_rightMesh = pos.clone().add(new THREE.Vector3(PartWall_width, 0 ,0) );
        pos_adjMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_rightMesh, tol_01);
    } else if (angle== -Math.PI/2) { 
        const pos_topMesh = pos.clone().add(new THREE.Vector3(0, PartWall_width,0) );
        pos_adjMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_topMesh, tol_01);
    }
...
// --------------------------------
//    Check Presence of 2 Adjacent Meshes
// --------------------------------
...
function checkPresenceOf2AdjacentMeshes( pos, angle ) {
    ...
    if (angle == 0 || angle == -Math.PI || angle == Math.PI){ // if mesh is horizontal
        ...
        const bool_PresenceofLeftMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_leftMesh, tol_01) !=undefined;

        const pos_rightMesh = pos.clone().add(new THREE.Vector3(PartWall_width, 0 ,0) );
        const bool_PresenceofRightMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_rightMesh, tol_01) !=undefined;

        bool_PresenceOf2AdjacentMeshes = bool_PresenceofLeftMesh && bool_PresenceofRightMesh

    } else { // if mesh is vertical
        const pos_topMesh = pos.clone().add(new THREE.Vector3(0, PartWall_width ,0) );
        const bool_PresenceofTopMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_topMesh, tol_01) !=undefined;

        const pos_bottomMesh = pos.clone().add(new THREE.Vector3(0, -PartWall_width ,0) );
        const bool_PresenceofBottomMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos_bottomMesh, tol_01) !=undefined;
...
// --------------------------------
//    Check Presence of Mesh within Tolerance 2 Adjacent Keys ★
// --------------------------------

function checkWithinTol_PresenceAndPosOfMesh(list_dict, pos_mesh, tolerance){ // e.g (list_dictVert, pos_meshPartWall, tol_01)
    var list_pos = []; 
    list_dict.forEach( function(dict){
        Object.keys(dict).forEach(i => {
            if ( dict[i].storey == storeyNum ){
                const pos = posfromKey( i )
                list_pos = list_pos.concat(pos) // console.log(i.children) // e.g. (9) [Mesh, Mesh, Mesh, Mesh..]
            }
        })
    })
    const closest_pos = getClosestPos(pos_mesh, list_pos );
...
function findMeshDictAtPos ( pos ) {
    const key = keyGen( pos );
    list_dictVert.foreach( function(dict){
        if (key in dict) {
            return dict
        }
    })
...
function get2AdjKeys(angle, pos) {
    var list_2AdjPos = [];
    if (angle == 0 || angle == Math.PI || angle == -Math.PI){ // if mesh is horizontal
        const pos_leftMesh = pos.clone().add(new THREE.Vector3(-PartWall_width, 0 ,0) );
        const pos_rightMesh = pos.clone().add(new THREE.Vector3(PartWall_width, 0 ,0) );
        list_2AdjPos = [pos_leftMesh, pos_rightMesh];
    } else { // if mesh is vertical
        const pos_topMesh = pos.clone().add(new THREE.Vector3(0, PartWall_width ,0) );
        const pos_bottomMesh = pos.clone().add(new THREE.Vector3(0, -PartWall_width ,0) );
        list_2AdjPos = [pos_topMesh, pos_bottomMesh];
    }
    var list_2AdjKeys = [];
    list_2AdjPos.forEach(function(pos){
        const closest_posMesh = checkWithinTol_PresenceAndPosOfMesh(list_dictVert, pos, tol_01); // account for tolerance
        if (closest_posMesh !== undefined){
            const key = keyGen( closest_posMesh );
            list_2AdjKeys.push(key);
        }
    })

    return list_2AdjKeys
}
...
function ifMatchKey_deleteMesh(key, ...args) { // Perform action to value (delete mesh) if it has the matching key
    ...
    if (args.includes('Window03')) {
        ifMatchKey_deleteWindow03(key)
    }
...
function ifMatchKey_deleteWindow03(key) {
    if (key in dictWindow03) {
        deleteWindow03(dictWindow03[key])
    }
}
function ifMatchKey_deleteWindow04(key) {
    if (key in dictWindow04) {
...
function  getAttrLineAdjacencyAtPos(pos) {
    const pos_AttrLine_extrap = new THREE.Vector3( pos.x, pos.y, zpos_AttrLine )
    const list_posAttrLine = getPos_allAttrLine ();
    const pos_closestAttrLine = getClosestPos (pos_AttrLine_extrap, list_posAttrLine); // CAN BE OPTIMISED
